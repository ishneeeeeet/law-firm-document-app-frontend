...
export default function MyDeal() {
  ...
  function snakeCaseToTitleCase(snakeCaseText) {
      ...
      <Container fluid>
                      ...
                      <StyledTableCell key={JSON.stringify(row.id + index)} align="center">
                        {
                          row.fileData ?
                            // Call uploadDocs when the button is clicked
                            <Button onClick={() => row.fileData ? update(row) : null} type="button" color="secondary">Update</Button>
                            :
                            <Button
                              color="light"
                              className='disabled'
        ...
        >
          ...
          <div className="modal-body">
            <form>
              <Row>
                {Object.entries(dealData).map(([label, value]) => {
                  {
                    return (label != "filePaths" && label != "jobId" && label != "dealtype") ? (
                      <div className="flex flex-wrap -mx-3 mb-6 col-md-6 ">
                        <div className="w-full px-3 mb-6 md:mb-0">
                          <Label className="form-label" for="grid-first-name">
                            {snakeCaseToTitleCase(label)}
                          </Label>
                          <input onChange={(data) => handleChange(data, label)} value={value} className="form-control" id={label} type="text" placeholder="" />
